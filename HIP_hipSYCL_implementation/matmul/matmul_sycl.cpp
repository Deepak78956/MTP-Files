...
clock_t matrix_mutilply(sycl::queue &q, int *A, int *B, int *C, unsigned int N)
{
    ...
    q.submit([&](sycl::handler &h) {
        h.parallel_for(sycl::nd_range<2>(sycl::range<2>{N, N}, sycl::range<2>{THREADS, THREADS}), [=](sycl::nd_item<2> itemID) {
...
void initZero(sycl::queue &q, int *C) {
    q.submit([&](sycl::handler &h) {
        h.parallel_for(sycl::nd_range<2>(sycl::range<2>{N, N}, sycl::range<2>{THREADS, THREADS}), [=](sycl::nd_item<2> itemID) {
            int i = itemID.get_global_id(0);
            int j = itemID.get_global_id(1);
