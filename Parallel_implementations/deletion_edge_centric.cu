...
__global__ void del_edges(struct EdgeContainer *dev_edgeContainer, int size, struct Graph *graph)
{
    ...
    {
        int u, v;
        u = dev_edgeContainer[id].u;
        v = dev_edgeContainer[id].v;

        // printf("thread id = %d, u = %d, v = %d\n", id, u, v);
...
__global__ void edgeFillCheck(struct EdgeContainer *dev_edgeContainer, int size)
{
    for (int i = 0; i < size; i++)
    {
        printf("%d %d\n", dev_edgeContainer[i].u, dev_edgeContainer[i].v);
    }
}
...
int main()
{
    ...
    // Bulk Delete start using edge centric
    ifstream fdel("file.txt");
    fdel >> num_vertices >> total_edges >> directed >> weighted;
    int size = 2 * total_edges;

    struct EdgeContainer *edgeContainer;
    edgeContainer = (struct EdgeContainer *)malloc(sizeof(struct EdgeContainer) * size);
    ...
    for (int i = 0; i < total_edges; i++)
    {
        ...
        edgeContainer[total_edges + i].u = v;
        edgeContainer[total_edges + i].v = u;
    }

    struct EdgeContainer *dev_edgeContainer;
    cudaMalloc(&dev_edgeContainer, sizeof(struct EdgeContainer) * size);
    cudaMemcpy(dev_edgeContainer, edgeContainer, sizeof(EdgeContainer) * size, cudaMemcpyHostToDevice);

    unsigned nBlocks_for_edges = ceil((float)size / BLOCK_SIZE);
    del_edges<<<nBlocks_for_edges, BLOCK_SIZE>>>(dev_edgeContainer, size, graph);
    cudaDeviceSynchronize();
