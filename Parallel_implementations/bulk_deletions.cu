...
__global__ void delEdges(struct Graph *graph, int *dev_offset_arr, int *dev_edge_list, int num_vertices)
{
    unsigned id = blockIdx.x * blockDim.x + threadIdx.x;
    if (id < num_vertices)
    {
        for (unsigned j = dev_offset_arr[id]; j < dev_offset_arr[id + 1]; j++)
        {
            int toDel = dev_edge_list[j];
...
int main()
{
    ...
    for (int i = 0; i < total_edges; i++)
    {
        ...
        if (!directed)
        {
            edges[total_edges + i][0] = v;
            edges[total_edges + i][1] = u;
        }
    }
    ...
    cudaMemcpy(dev_offset_arr, csr_del.offsetArr, sizeof(int) * (num_vertices + 1), cudaMemcpyHostToDevice);
